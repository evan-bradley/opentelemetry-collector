...
type Traces interface {
	consumer[ptrace.Traces]

	// deprecated: use Traces.Consume instead
	ConsumeTraces(ctx context.Context, md ptrace.Traces) error
}

type baseTraces struct {
	*baseConsumer[ptrace.Traces]
}

func (m baseTraces) ConsumeTraces(ctx context.Context, md ptrace.Traces) error {
...
func NewTraces(consume ConsumeTracesFunc, options ...Option) (Traces, error) {
	...
	bc := c.(baseConsumer[ptrace.Traces])
	return &baseTraces{
		baseConsumer: &bc,
	}, nil
}
